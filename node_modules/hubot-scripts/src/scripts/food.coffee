...
try
 creditCard = new ordrin.CreditCard "Mastercard", 5, 2014, address, 5519618768429526, 444
catch e
 console.log e
 throw e
...
trayItem = (itemId, quantity) ->
  try
    item = new ordrin.TrayItem itemId, quantity
  catch e
    console.log e
    throw e
...
module.exports = (robot) ->
  ...
  order_prefix = process.env.HUBOT_ORDER_PREFIX ? 'ordrin'
  make_regex = (regex_string) ->
    new RegExp "^#{order_prefix} ?#{regex_string}"

  robot.hear make_regex("fuzz me ([^ ]*) (.*)",'i'), (msg) ->
  ...
  robot.hear make_regex("check it", 'i'), (msg) ->
    state.confirmed = true
    msg.send print_order state.restaurant, state.orders

  robot.hear make_regex("it", 'i'), (msg) ->
    if not state.confirmed
      ...
      total = 0
      try
        tray = new ordrin.Tray([])
      catch e
        console.log e
        throw e

      msg.send pretty_json tray
      for order in state.orders
        total += parseFloat order.item.price
        tray.items.push trayItem (parseInt itemId), 1
  ...
  robot.hear make_regex("edit distance ([^ ]*) (.*)",'i'), (msg) ->
