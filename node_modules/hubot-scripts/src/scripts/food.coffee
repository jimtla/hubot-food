...
print_order = (restaurant, orders) ->
  lines = [
    ...
    for order, index in orders
      "#{index+1}. #{order.item.name} (Ordered by #{order.orderer})"
  ]
  _(lines).flatten().join '\n'
...
module.exports = (robot) ->
  ...
  get_item_fuzzy = (fuzz, menu, msg, pretty, c) ->
    get_obj_fuzzy fuzz, (_.flatten _.pluck menu, 'children'), msg, pretty, c,
      -> msg.send "See the menu above for options.",
      (i) -> [i.name]
  ...
  respond_to "it", (msg) ->
    if not state.confirmed
      ...
      for order in state.orders
        ...
        tray.items.push tryWithLog -> new ordrin.TrayItem (parseInt order.item.id), 1
      ...
      ordrin.restaurant.getDeliveryList new Date, address, unlessErr msg, (rs) ->
        get_restaurant_fuzzy msg.match[1], rs, msg, print_restaurants, (r) ->
          ordrin.restaurant.getDetails r.id, unlessErr msg, (r) ->
            state.restaurant = r
            msg.send "@all Ordering from #{r.name}! To add your food to the order: #{ordrin_prefix} <menu item>"
