...
tryWithLog = (f) ->
  try
    ...
    throw e
address = tryWithLog ->
  new ordrin.Address "841 Broadway", "New York", "NY", 10014, 4014641931
...
module.exports = (robot) ->
  ...
  get_obj_fuzzy = (fuzz, arrayOfObjWithIds, msg, pretty, c, getObjKeywords) ->
    fuzz = fuzz.trim()
    keywordToIds = {}
    ...
    if objs.length is 1
      ...
      msg.reply "#{objs.length} matches for `#{fuzz}` - be more specific?\n"
  ...
  robot.hear new RegExp("^#{process.env.HUBOT_ORDER_PREFIX ? 'ordrin'}(.*)", "i"), (msg) ->
  ...
  respond_to "it", (msg) ->
    if not state.confirmed
      ...
      for order in state.orders
        ...
        tray.items.push tryWithLog -> new ordrin.TrayItem (parseInt itemId), 1
  ...
  respond_to "(.*)", (msg) ->
    if state.restaurant? # Choose a dish
      ...
      ordrin.restaurant.getDeliveryList new Date, address, unlessErr msg, (rs) ->
        get_restaurant_fuzzy msg.match[1], rs, msg, print_restaurants, (r) ->
          ordrin.restaurant.getDetails r.id, unlessErr msg, (r) ->
            state.restaurant = r
            msg.send "Ordering from #{r.name}."
            msg.send "@all chat `ordrin <menu item>` to order."
            msg.reply "When all orders have been placed `ordrin it`!"
            msg.send print_menu r
